...
describe('convertToResearchStudy()', () => {
  ...
  it('maps as expected', () => {
    ...
    expect(researchStudy.identifier).toEqual([{ use: 'official', system: 'http://clinicaltrials.gov', value: trialResponse.trialId }]);
    ...
    expect(researchStudy.keyword).toEqual([
      { text: "METASTATIC" },
      { text: "TREATMENT_BIOLOGICAL" }
    ]);
    ...
    if (researchStudy.sponsor) {
      ...
      expect(researchStudy.sponsor).toEqual({ reference: '#' + id, type: 'Organization' });
